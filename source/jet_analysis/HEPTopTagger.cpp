...
namespace analysis
{
	...
	HEPTopTagger::HEPTopTagger(const fastjet::ClusterSequence & cs, const fastjet::PseudoJet & jet) : 
		debugg(false),
	...
	HEPTopTagger::HEPTopTagger(const fastjet::ClusterSequence & cs, const fastjet::PseudoJet & jet, double mtmass,double mwmass) : 
		debugg(false),
	...
	void HEPTopTagger::get_info() const
	{
		...
		std::cout << "top candidate (pt, eta, phi): (" <<  _top_candidate.perp() << "," <<  _top_candidate.eta() << "," <<  _top_candidate.phi_std() << ")" << std::endl;
	...
	void HEPTopTagger::get_setting() const
	{
		...
		std::cout << "W mass ratio range: [" << _rmin << ", " << _rmax << "] ([" <<_rmin*_mtmass/_mwmass<< "%, "<< _rmax*_mtmass/_mwmass << "%])"<< std::endl;
		std::cout << "mass ratio cut: (m23cut, m13min, m13max)=(" << _m23cut << ", " << _m13cutmin << ", " << _m13cutmax << ")" << std::endl;
	...
	bool HEPTopTagger::check_mass_criteria(const std::vector<fastjet::PseudoJet> & top_subs) const
	{
		...
		double m23=(top_subs[1]+top_subs[2]).m();
		double m123=(top_subs[0]+top_subs[1]+top_subs[2]).m();
		if ( (atan(m13/m12)>_m13cutmin && _m13cutmax > atan(m13/m12) && (m23/m123>_rmin && _rmax>m23/m123))
			|| (((m23/m123)*(m23/m123) < 1-_rmin*_rmin*(1+(m13/m12)*(m13/m12))) && ((m23/m123)*(m23/m123) > 1-_rmax*_rmax*(1+(m13/m12)*(m13/m12))) && (m23/m123 > _m23cut))
			|| (((m23/m123)*(m23/m123) < 1-_rmin*_rmin*(1+(m12/m13)*(m12/m13))) && ((m23/m123)*(m23/m123) > 1-_rmax*_rmax*(1+(m12/m13)*(m12/m13))) && (m23/m123 > _m23cut)) )
		{ 
			is_passed = true;
		}
		return is_passed;
