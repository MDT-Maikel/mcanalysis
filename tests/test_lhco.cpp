...
int main(int argc, const char* argv[])
{
	...
	vector<event*> events;
	if (is_regular_file("../../files/tests/input/test_lhco_events.lhco.gz"))
	{
		read_lhco(events, "../../files/tests/input/test_lhco_events.lhco.gz");
	}
	else
	{
		cout << "File (../../files/tests/input/test_lhco_events.lhco.gz) for reading lhco events not available." << endl;
	...
	if (test_reading_passed)
	{
		write_lhco(events, "../../files/tests/output/test_lhco_events.lhco.gz");
		
		// test whether the writing has succeeded
		if (is_regular_file("../../files/tests/output/test_lhco_events.lhco.gz"))
		{
			// check file size of the newly created lhco file 
			// has to be within 90 to 110 percent of old file
			unsigned int in_size = file_size("../../files/tests/input/test_lhco_events.lhco.gz");
			unsigned int out_size = file_size("../../files/tests/output/test_lhco_events.lhco.gz");
			...
			remove("../../files/tests/output/test_lhco_events.lhco.gz");
		}
	}
